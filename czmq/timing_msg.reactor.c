...
{
    int rc;
    rc = pthread_create(&ORUCli_Tid, NULL,(void *)Timing_Msg_Task_Entry, NULL);
    if (rc) {
        log_err("%s[%d], create Timing_Msg_Task_Entry pthread failed", __FUNCTION__, __LINE__);
        return -1;
    }
